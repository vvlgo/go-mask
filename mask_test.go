...
func (t *LocalTime) UnmarshalJSON(arr []byte) error {
	tVal, err := time.Parse("\"2006-01-02 15:04:05\"", string(arr))
	if err != nil {
		return err
	}
	*t = LocalTime(tVal)
...
func (t LocalTime) DateString() string {
	tTime := time.Time(t)
	return fmt.Sprintf(`%v`, tTime.Format("2006-01-02"))
}
func (t LocalTime) String() string {
	tTime := time.Time(t)
	return fmt.Sprintf(`%v`, tTime.Format("2006-01-02 15:04:05"))
}
func TestTime(t *testing.T) {
	...
	fmt.Printf("%+v", maskUser)
	marshal, err := json.Marshal(maskUser)
